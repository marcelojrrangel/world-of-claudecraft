...
const entrySource = `
  ...
  export { BLUEPRINTS } from './src/sim/content/blueprints.ts';
  export { CONSTRUCTION_ITEMS } from './src/sim/content/construction_items.ts';
...
// ---- Construction data ----
const FURNITURE_SETS_DATA = [
  { setId: 'rustic', quality: 'poor', label: 'Rustic', minTier: 1, items: ['chair', 'table', 'bed', 'shelf', 'rug', 'lamp', 'cabinet'] },
  { setId: 'sturdy', quality: 'common', label: 'Sturdy', minTier: 2, items: ['chair', 'table', 'bed', 'shelf', 'rug', 'lamp'] },
  { setId: 'ornate', quality: 'uncommon', label: 'Ornate', minTier: 3, items: ['chair', 'table', 'bed', 'cabinet', 'lamp', 'rug'] },
  { setId: 'exquisite', quality: 'rare', label: 'Exquisite', minTier: 4, items: ['chair', 'table', 'bed', 'lamp'] },
  { setId: 'masterwork', quality: 'epic', label: 'Masterwork', minTier: 5, items: ['chair', 'table', 'bed', 'rug'] },
];
const STATIONS_DATA = [
  { id: 'station_workbench', name: 'Workbench' },
  { id: 'station_anvil', name: 'Anvil' },
  { id: 'station_alchemy', name: 'Alchemy Station' },
  { id: 'station_cooking_fire', name: 'Cooking Fire' },
  { id: 'station_loom', name: 'Loom' },
];
const CHEST_DATA = [
  { id: 'chest_small', name: 'Small Chest', quality: 'common' },
  { id: 'chest_medium', name: 'Medium Chest', quality: 'uncommon' },
  { id: 'chest_large', name: 'Large Chest', quality: 'rare' },
];
...
const blueprints = BLUEPRINTS.map((b) => ({
  ...
  name: b.name,
  tier: b.tier,
  requiredSkill: b.requiredSkill,
  itemId: b.itemId,
  phases: b.phases.map((p) => ({
    nameId: p.nameId,
    materials: p.materials.map((m) => ({ itemId: m.itemId, count: m.count })),
    toolTier: p.toolTier,
    skillGain: p.skillGain,
    trivialAt: p.trivialAt,
  })),
}));

const furnitureSets = FURNITURE_SETS_DATA.map((s) => ({
  setId: s.setId,
  label: s.label,
  quality: s.quality,
  minTier: s.minTier,
  items: s.items.map((item) => {
    const fullId = `${s.setId}_${item}`;
    const def = CONSTRUCTION_ITEMS[fullId];
    return { id: fullId, name: def?.name ?? fullId };
  }),
}));

const stations = STATIONS_DATA.map((s) => ({ id: s.id, name: s.name }));
const chests = CHEST_DATA.map((c) => ({ id: c.id, name: c.name, quality: c.quality }));
...
writeFileSync(
  ...
  [
    ...
    `\nexport const GUIDE_BLUEPRINTS: GuideBlueprint[] = ${JSON.stringify(blueprints, null, 2)};\n`,
    `\nexport const GUIDE_FURNITURE_SETS: GuideFurnitureSet[] = ${JSON.stringify(furnitureSets, null, 2)};\n`,
    `\nexport const GUIDE_CRAFT_STATIONS: GuideCraftStation[] = ${JSON.stringify(stations, null, 2)};\n`,
    `\nexport const GUIDE_CHESTS: GuideChestItem[] = ${JSON.stringify(chests, null, 2)};\n`,
  ].join(''),
);
// eslint-disable-next-line no-console
console.log(
  `generated src/guide/content.generated.ts (${classes.length} classes, ${zones.length} zones, ${dungeons.length} dungeons, ${warlockPets.length} warlock pets, ${families.length} families, ${delves.length} delves, ${Object.keys(MODELS).length} models, ${blueprints.length} blueprints, ${furnitureSets.length} furniture sets, ${stations.length} stations, ${chests.length} chests)`,
);
